...
class DialogService:
    ...
    async def list_dialogs(self, limit: int = 50, offset: int = 0) -> ListDialogsResponse:
        ...
        dialogs = await self.client_manager.call(_load, action="list dialogs")
        sliced = dialogs[offset : offset + limit]
        items = [self._dialog_info(dialog) for dialog in sliced]
        return ListDialogsResponse(items=items, total=len(dialogs), limit=limit, offset=offset)

    async def get_dialog(self, dialog_id: int) -> DialogInfo:
        entity = await self.chat_resolver.resolve(dialog_id)
        dialogs = await self.client_manager.call(
            lambda client: client.get_dialogs(),
            action=f"get dialog {dialog_id}",
        )
